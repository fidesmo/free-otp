...
public class ExternalToken implements Token {
    ...
    public ExternalToken(TokenMeta meta) {
    ...
    private TokenCode generateTotpCode(HardwareToken token, long counter, TokenCode next) throws IOException {
        return new TokenCode(token.readTotpCode(id, counter), counter * period * 1000, (counter + 1) * period * 1000, next);
    }

    private TokenCode generateHotpCode(HardwareToken token, long cur) throws IOException {
        return new TokenCode(token.readHotpCode(id), cur, cur + (period * 1000));
    }

    public TokenCode generateCodes(HardwareToken token) throws IOException {
        long cur = System.currentTimeMillis();
        switch(type) {
        case TOTP:
            long counter = cur / 1000 / period;
            return generateTotpCode(token, counter, generateTotpCode(token, counter + 1, null));
        case HOTP:
            return generateHotpCode(token, cur);
        default:
