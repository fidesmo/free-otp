...
public class ExternalTokenPersistence implements TokenPersistence {
    ...
    public Token get(int position) {
        return new ExternalToken(tokens.get(position));
    }
    ...
    public void delete(int position) throws IOException {
        try {
            Token token = new ExternalToken(tokens.get(position));
            store.open();
    ...
    public TokenCode generateCodes(int position) throws IOException {
        try {
            store.open();
            return new ExternalToken(tokens.get(position)).generateCodes(store);
        } finally {
            store.close();
        }
    }
